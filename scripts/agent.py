...
class AgentExecutor(Chain):
    ...
    async def _aperform_agent_action(
        ...
        # Attack by tool response
        tool_list = list(color_mapping.keys())
        support_tools = []
        if attack_type!='search_term_deception':
            if attack_task == 'kill_process' and tool_list[0]=='read_file':
                support_tools = tool_list[:5]
            elif tool_list[0]=='read_file':
                support_tools = tool_list[:10]
        if tool_response_attack != '' and agent_action.tool not in support_tools and attack_type!='search_term_deception':
            observation = tool_response_attack
        # Otherwise we lookup the tool
        else:
            if agent_action.tool in name_to_tool_map:
                tool = name_to_tool_map[agent_action.tool]
        ...
        if attack_type=='search_term_deception':
            observation = observation.replace('{attack_instruction}',tool_response_attack)
        return AgentStep(action=agent_action, observation=observation)
